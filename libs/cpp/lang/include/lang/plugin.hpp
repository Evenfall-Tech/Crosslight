...
public:
    ...
    plugin(const std::string &dir_path, const std::string &lib_name, bool decorations = add_filename_decorations)
        : plugin(dir_path.c_str(), lib_name.c_str(), decorations) {}

    plugin(const std::string &dir_path, const char *lib_name, bool decorations = add_filename_decorations)
        : plugin(dir_path.c_str(), lib_name, decorations) {}

    plugin(const char *dir_path, const std::string &lib_name, bool decorations = add_filename_decorations)
        : plugin(dir_path, lib_name.c_str(), decorations) {}

    explicit plugin(const std::string &lib_name, bool decorations = add_filename_decorations)
        : plugin("", lib_name.c_str(), decorations) {}

    explicit plugin(const char *lib_name, bool decorations = add_filename_decorations)
        : plugin("", lib_name, decorations) {}

#ifdef DYLIB_CPP17
    explicit plugin(const std::filesystem::path &lib_path)
        : plugin("", lib_path.string().c_str(), no_filename_decorations) {}

    plugin(const std::filesystem::path &dir_path, const std::string &lib_name, bool decorations = add_filename_decorations)
        : plugin(dir_path.string().c_str(), lib_name.c_str(), decorations) {}

    plugin(const std::filesystem::path &dir_path, const char *lib_name, bool decorations = add_filename_decorations)
        : plugin(dir_path.string().c_str(), lib_name, decorations) {}
#endif
    ///@}
