...
namespace Crosslight.Common.UI.Controls.HexEditorControl.Core.CharacterTable
{
    /// <summary>
    ...
    public sealed class TblStream : IDisposable
    {
        ...
        public (string text, DteType dteType) FindMatch(string hex, bool showSpecialValue)
        {
            if (showSpecialValue)
            {
                if (_dteList.ContainsKey($"/{hex}")) return (Localizer.Localizer.Instance["HexEditor/EndTagString"], DteType.EndBlock); //"<end>";
                if (_dteList.ContainsKey($"*{hex}")) return (Localizer.Localizer.Instance["HexEditor/LineTagString"], DteType.EndLine); //"<ln>";
            }
        ...
        {
            if (tblString.StartsWith("avares://"))
            {
                var stream = AvaloniaLocator.Current.GetService<IAssetLoader>().Open(new Uri(tblString));
                stream.Position = 0;
                using (StreamReader reader = new StreamReader(stream, Encoding.UTF8))
                {
                    tblString = reader.ReadToEnd();
                }
            }
            //Variables
            char[] sepEndLine = { '\n' }; //end line char
        ...
        public static TblStream CreateDefaultTbl(DefaultCharacterTableType type = DefaultCharacterTableType.Ascii)
        {
            ...
            switch (type)
            {
                ...
                case DefaultCharacterTableType.EbcdicWithSpecialChar:
                    tbl.Load(Localizer.Localizer.Instance["HexEditor/EBCDIC"]);
                    break;
                case DefaultCharacterTableType.EbcdicNoSpecialChar:
                    tbl.Load(Localizer.Localizer.Instance["HexEditor/EBCDICNoSpecialChar"]);
                    break;
            }
